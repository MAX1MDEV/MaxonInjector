...
namespace MaxonInjector
{
    public partial class Form1 : PoisonForm
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                Process.Start("https://discord.gg/sf4Mh6qygm");
            }
        }
...
+ "", "[How to Use]");
        ...
        private void poisonToggle1_Click(object sender, EventArgs e)
        {
            if (poisonToggle1.Checked == false)
            {
                ...
                Properties.Settings.Default.Save();
                if (System.IO.File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"\MaxonInjector.lnk"))
                    System.IO.File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + @"\MaxonInjector.lnk");
            }
            ...
            if (poisonToggle1.Checked == true)
            {
                ...
                IWshRuntimeLibrary.IWshShortcut shortcut = wsh.CreateShortcut(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\MaxonInjector.lnk") as IWshRuntimeLibrary.IWshShortcut;
                shortcut.TargetPath = AppPath + @"\MaxonInjector.exe";
                shortcut.WindowStyle = 1;
                shortcut.WorkingDirectory = AppPath;
                shortcut.IconLocation = AppPath + @"\MaxonInjector.exe";
                shortcut.Save();
            }
        }
        ...
        {
            string url = "https://discord.gg/sf4Mh6qygm";
            try
            {
        ...
        private void poisonButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    PoisonMessageBox.Show(this, "Successfully injected!", "MaxonInjector: Injection", MessageBoxButtons.OK, MessageBoxIcon.Question);
                }
                else
                {
